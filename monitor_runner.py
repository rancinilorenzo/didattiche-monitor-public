...
def main() -> int:
    ...
    # Reminder 45 + 5.
    reminder_groups: dict[int, list[str]] = {}
    ...
    for lesson in new_lessons:
        ...
        chosen = min(due)
        reminder_groups.setdefault(chosen, []).append(
            reminder_card(
                lesson,
                minutes,
    ...
    for threshold in sorted(reminder_groups, reverse=True):
        core.notify(
            f"⏰ DIDATTICA IN ARRIVO · {threshold} MIN",
            "\n\n".join(reminder_groups[threshold]),
        )
